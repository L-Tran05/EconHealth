...
const BillDetailsPanel = ({ billText, setBillText, setActiveTab }) => (
  <div className="lg:col-span-3 bg-white rounded-lg left-10 shadow-sm border border-black-400 p-6 h-[420px]">
    ...
    <textarea
    /*Area where user inputs the medical bill of theirs*/
      value={billText}
      onChange={(e) => setBillText(e.target.value)}
      placeholder="Paste your medical bill text here..."
      className="w-full h-64 p-3 border border-black-300 rounded-lg font-mono text-xs text-black-700 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none mb-4"
    />

    <button
      onClick={() => billText.trim() && setActiveTab('recommendations')}
      className="w-full bg-blue-500 hover:bg-blue-600 text-white font-medium py-2.5 px-6 rounded-lg text-sm transition-colors"
    >
      Extract bill facts
    </button>
  </div>
